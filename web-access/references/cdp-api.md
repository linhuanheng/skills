...
- **自动关闭**：默认 5 分钟无活动后自动关闭，可通过环境变量 `CDP_PROXY_IDLE_TIMEOUT`（毫秒）调整
- **手动关闭**：`curl -s "http://localhost:$PORT/shutdown"`
- 进程信号：支持 `SIGINT` (Ctrl+C) 和 `SIGTERM` 优雅关闭
- 端口检查：启动时自动检测已有实例，避免重复启动
...
### GET /shutdown
关闭 CDP Proxy 实例。用于手动触发关闭，响应后延迟关闭以确保消息发送。
```bash
curl -s "http://localhost:3457/shutdown"
```
