...
async function detectPortConflict(preferredPort) {
  ...
  if (available) {
    return { port: preferredPort, conflict: false };
  }
  // 端口被占用，查找可用端口
  const newPort = await findAvailablePort(preferredPort + 1);
  return { port: newPort, conflict: true };
}

const PREFERRED_PORT = parseInt(process.env.CDP_PROXY_PORT) || DEFAULT_PORT;
let ws = null;
let cmdId = 0;
...
// 空闲超时配置
const IDLE_TIMEOUT_MS = parseInt(process.env.CDP_PROXY_IDLE_TIMEOUT) || (5 * 60 * 1000); // 默认5分钟
let idleTimer = null;
...
function resetIdleTimer() {
  ...
  idleTimer = setTimeout(() => {
    console.log(`[CDP Proxy] 空闲超时 ${IDLE_TIMEOUT_MS / 1000} 秒，自动关闭`);
    gracefulShutdown();
  }, IDLE_TIMEOUT_MS);
}
...
function gracefulShutdown() {
  ...
  // 如果server.close超时，强制退出
  setTimeout(() => {
    console.log('[CDP Proxy] 关闭超时，强制退出');
    process.exit(1);
  }, 5000);
}
...
// --- HTTP API ---
let actualListeningPort = DEFAULT_PORT; // 用于记录实际监听的端口号
const server = http.createServer(async (req, res) => {
  // 重置空闲计时器
  resetIdleTimer();

  const parsed = new URL(req.url, `http://localhost:${actualListeningPort}`);
  ...
  try {
    ...
    // GET /shutdown - 关闭代理
      ...
      res.end(JSON.stringify({ message: '正在关闭 CDP Proxy...' }));
      // 延迟关闭，确保响应发送
...
async function main() {
  // 检测端口冲突并自动切换到可用端口
  const portInfo = await detectPortConflict(PREFERRED_PORT);
  const actualPort = portInfo.port;

  if (portInfo.conflict && PREFERRED_PORT !== actualPort) {
    console.log(`[CDP Proxy] ⚠️  端口 ${PREFERRED_PORT} 已被占用，自动切换到端口 ${actualPort}`);
  }

  // 检查目标端口是否已有健康实例运行
  try {
    const ok = await new Promise((resolve) => {
      http.get(`http://127.0.0.1:${actualPort}/health`, { timeout: 2000 }, (res) => {
        let d = '';
        res.on('data', c => d += c);
    ...
    if (ok) {
      console.log(`[CDP Proxy] 已有实例运行在端口 ${actualPort}，退出`);
      process.exit(0);
    }
  } catch { /* 端口无响应，继续启动 */ }

  server.listen(actualPort, '127.0.0.1', () => {
    actualListeningPort = actualPort; // 保存实际监听端口
    console.log(`[CDP Proxy] 运行在 http://localhost:${actualPort}`);
    // 启动空闲计时器
    resetIdleTimer();
    // 启动时尝试连接 Chrome（非阻塞）
