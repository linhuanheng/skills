...
**v2.5.0 更新：**
- **自动关闭功能** — 默认 5 分钟空闲超时自动关闭 Proxy，支持手动关闭（`/shutdown` 端点）和信号处理，优化资源占用
- **新增 API** — 新增 `/shutdown` 端点用于手动关闭 Proxy 实例
...
**自动管理**：
- **空闲超时**：默认 5 分钟无活动后自动关闭，可通过 `CDP_PROXY_IDLE_TIMEOUT` 环境变量调整
- **手动关闭**：`curl -s "http://localhost:$PORT/shutdown"`
- **端口检查**：自动检测已有实例，避免重复启动
...
curl -s "http://localhost:3457/shutdown"                                    # 手动关闭 Proxy
```
