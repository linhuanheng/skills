...
检查脚本会自动检测 CDP Proxy 端口（默认 3457）是否被占用：

- **无冲突**：使用默认端口 3457
- **有冲突**：自动切换到下一个可用的闲置端口（如 3458、3459...，最多尝试 10 个连续端口，若仍被占用则在 8000-8100 范围内查找）
- **输出**：脚本会输出实际使用的端口号 `CDP_PROXY_PORT=xxxx`

如果检测到端口冲突并自动切换，会输出提示信息：
```
⚠️  端口 3457 已被占用，自动切换到端口 3458
```

**重要**：后续所有 CDP Proxy API 调用必须使用环境变量 `CDP_PROXY_PORT` 获取实际端口号，例如：
```bash
PORT="${CDP_PROXY_PORT:-3457}"  # 使用环境变量，默认 3457
curl -s "http://localhost:$PORT/targets"
```
...
**端口冲突处理**：
- 脚本会自动检测默认端口 3457 是否被占用
- 如有冲突，自动切换到下一个可用的闲置端口（3458、3459...）
- 输出会显示实际使用的端口号，例如：`CDP_PROXY_PORT=3458`
- 后续所有 API 调用必须使用环境变量获取端口号：`PORT="${CDP_PROXY_PORT:-3457}"`

### Proxy API

所有操作通过 curl 调用 HTTP API。**必须使用环境变量 `CDP_PROXY_PORT` 获取实际端口号**（初始化的脚本会自动设置该环境变量）。

```bash
# 必须使用环境变量获取端口号（默认 3457，冲突时自动切换）
...
# 创建新后台 tab（自动等待加载）
curl -s "http://localhost:$PORT/new?url=https://example.com"

# 页面信息
curl -s "http://localhost:$PORT/info?target=ID"

# 执行任意 JS：可读写 DOM、提取数据、操控元素、触发状态变更、提交表单、调用内部方法
curl -s -X POST "http://localhost:$PORT/eval?target=ID" -d 'document.title'

# 捕获页面渲染状态（含视频当前帧）
curl -s "http://localhost:$PORT/screenshot?target=ID&file=/tmp/shot.png"

# 导航、后退
curl -s "http://localhost:$PORT/navigate?target=ID&url=URL"
curl -s "http://localhost:$PORT/back?target=ID"

# 点击（POST body 为 CSS 选择器）— JS el.click()，简单快速，覆盖大多数场景
curl -s -X POST "http://localhost:$PORT/click?target=ID" -d 'button.submit'

# 真实鼠标点击 — CDP Input.dispatchMouseEvent，算用户手势，能触发文件对话框
curl -s -X POST "http://localhost:$PORT/clickAt?target=ID" -d 'button.upload'

# 文件上传 — 直接设置 file input 的本地文件路径，绕过文件对话框
curl -s -X POST "http://localhost:$PORT/setFiles?target=ID" -d '{"selector":"input[type=file]","files":["/path/to/file.png"]}'

# 滚动（触发懒加载）
curl -s "http://localhost:$PORT/scroll?target=ID&y=3000"
curl -s "http://localhost:$PORT/scroll?target=ID&direction=bottom"

# 关闭 tab
curl -s "http://localhost:$PORT/close?target=ID"
```
...
为了优化资源使用，Proxy 新增了自动关闭功能：

- **空闲超时**：默认 5 分钟无活动后自动关闭 Proxy。可通过环境变量 `CDP_PROXY_IDLE_TIMEOUT`（毫秒）调整
- **手动关闭**：通过 `/shutdown` 端点手动关闭 `curl -s "http://localhost:$PORT/shutdown"`
- **进程信号**：支持 `SIGINT` (Ctrl+C) 和 `SIGTERM` 信号优雅关闭
- **端口检查**：启动时自动检查是否已有 Proxy 实例运行，避免重复启动
...
**注意**：
- 关闭后重启需要 Chrome 重新授权 CDP 连接（取决于 `chrome://inspect` 设置）
- 关闭命令必须使用正确的端口号：`PORT="${CDP_PROXY_PORT:-3457}" && curl -s "http://localhost:$PORT/shutdown"`
